...
│   │   ├── imageTools.ts       # 图片处理工具（402 行，核心图片功能）
│   │   ├── monaco.ts           # Monaco Editor 配置
│   │   └── textTools.ts        # 文本处理工具
│   │
│   ├── views/                  # 页面视图
│   │   ├── HomeView.vue        # 首页（产品介绍页）
│   │   └── WorkspaceView.vue   # 工作区页面（主功能页，1650 行）
│   │
│   ├── App.vue                 # 根组件
│   ├── main.ts                 # 应用入口
│   ├── style.less              # 全局样式
...
- **`src/views/WorkspaceView.vue`** (1650 行)
  - 主工作区视图，包含所有工具的核心逻辑
  - 管理 JSON、文本、图片三种工具模式
  - 处理编辑器实例、状态管理、操作处理
...
项目采用 Vue 3 Composition API + TypeScript 架构，主要特点：

1. **组件化设计**：功能模块高度组件化，便于维护和扩展
2. **类型安全**：全面使用 TypeScript，提供完整的类型定义
3. **状态管理**：使用 Vue 3 响应式系统管理状态
4. **工具函数分离**：工具函数独立封装，便于测试和复用
